describe( 'AppCtrl', function() {
  ...
  describe('test flightSearchProjectService', function() {
    ...
    it('should return the correct results when user do a searchFlight query', function() {
      ...
      flightSearchService.searchFlight(from, to, adult, date, scope);
      httpBackend.flush();
      ...
      expect(flightResults[0]["price"]).toBe("SGD790.70");
      expect(flightResults[0]["carrier"]).toBe("Air India (AIC)");
      expect(flightResults[0]["departure"]).toBe("2016-11-19T08:25+08:00");
      expect(flightResults[0]["arrival"]).toBe("2016-11-19T18:55+01:00");
      expect(flightResults[0]["stops"]).toBe(1);
    });
    ...
    it('should return error message if the flight cannot be found', function() {
      ...
      spyOn(growl, "error");
      flightSearchService.searchFlight(from, invalidTo, adult, date, scope);
      httpBackend.flush();

      var flightResults = flightSearchService.getFlightResult();
      expect(flightResults.length).toBe(0);
      expect(growl.error).toHaveBeenCalledWith("No trips can't be found, please make sure that you have entered a valid city name");
    });
  });
});
